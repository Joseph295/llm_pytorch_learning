...
4. **验证数据**：检查 token id 越界（无）、batch/init 校验和（与稳定版逐位相同，排除数据/RNG）；
5. **关键突破**：注意到 gnorm 异常高（5~13，健康应 <1）。Adam 的 **β₂=0.95** 让二阶矩快速适应——遇偶发大梯度时分母 √v 骤降、放大下一步。换回默认 **β₂=0.999**（二阶矩平滑），跑 3 个随机种子——**全部稳定，gnorm 回到 ~1**。根因锁定。

**根因**：β₂=0.95 是 LLaMA 等**大模型 + 大 batch**的配置（第 6 章 6.5-②）。小模型（12M）+ 小 batch（16）梯度噪声大，0.95 的快速二阶矩适应放大噪声致失稳。**同一超参，大模型良药、小模型毒药**——照抄大模型配置的经典陷阱。

**修复（多管齐下，通用于任何 loss spike）**：
- **平滑 Adam β₂**（0.95 → 0.999）：**根因修复**，二阶矩不放大偶发大梯度；
- **更紧的梯度裁剪**（clip 1.0 → 0.5）：限制单步伤害；
- **更保守的 lr + 更长 warmup**：留稳定裕度；
- **按验证集存最优 checkpoint**（而非存最后一步）：即便偶发尖峰，也保证最终拿到好模型——**最关键的工程保险**。

**教训**：① loss spike 的根因可能藏在最不起眼的超参里（谁会先怀疑 β₂？）——但高 gnorm 是它的指纹，监控 gnorm 就能顺藤摸瓜；② 大模型的配置不能无脑照抄到小模型（β₂/lr/warmup 都随规模变）；③ 最可靠的防御不是消除所有尖峰，而是"最优 checkpoint 保存"这类让偶发故障不影响最终产物的设计；④ 大规模训练同理——loss spike 常见，工业做法是监控 gnorm + 自动回滚到 spike 前 checkpoint。
