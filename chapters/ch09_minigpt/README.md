...
- 冒烟后（几十步）：纯乱码/高频字堆砌；
- 500 步：开始出现成词、标点位置合理；
- 训练完成：短句成形、有《红楼梦》的文白腔调（虽然逻辑仍会漂移——12M 参数 + 60 万 token 的能力上限，别期待 GPT-4）。

**本项目实测基线**（M4，12.2M 参数，β₂=0.999 稳定配置，~15 分钟）：
- loss 从 8.41（≈ln 4096）平稳降到 train 3.8 / 最优 val 6.34（gnorm 全程 1~3.7，健康）；
- best-checkpoint 落在 step 1500（val 最低），之后 val 上升是过拟合（train 仍降）；
- 用最优 checkpoint 生成（`--prompt 黛玉`）的真实输出：

> 黛玉才要問，香菱听了，只得喝一口．二人又指家四一說……黛玉冷笑道："快別叫作'好姐姐'的'二字．寶丫頭道："小丫頭都不理論，這件東西雖無名，我已知道了多少話……"

真实人名（黛玉/香菱/寶丫頭）、对话结构（X道："…"）、文白腔调都学到了——逻辑会漂移，但**风格**清晰可辨。一个你从零手写、在笔记本上训出来的语言模型。
